    ...
    <xsl:template match="system-index-block">
        <xsl:variable name="content-type">
            ...
            <xsl:choose>
                ...
                <xsl:when test="calling-page/system-page/system-data-structure/content-type-detail"
                    >detail</xsl:when>
                <xsl:when
                    test="calling-page/system-page/system-data-structure/content-type-directory"
                    >directory</xsl:when>
                <xsl:when
                    test="calling-page/system-page/system-data-structure/content-type-machform"
                    >machform</xsl:when>
                <xsl:when test="calling-page/system-page/system-data-structure/content-type-section"
                    >section</xsl:when>
                <xsl:when test="calling-page/system-page/system-data-structure/content-type-map"
                    >map</xsl:when>
                <xsl:when test="calling-page/system-page/system-data-structure/content-type-video"
                    >video</xsl:when>
        ...
        <xsl:for-each select="system-block[system-data-structure/redesign]">
            ...
            <xsl:variable name="no-top-ribbon">
                <xsl:choose>
                    <xsl:when
                        test="system-data-structure/redesign/nav-options/no-top-ribbon/value = 'Yes'">
            ...
            <xsl:variable name="no-top-navigation">
                <xsl:choose>
                    <xsl:when
                        test="system-data-structure/redesign/nav-options/no-top-navigation/value = 'Yes'">
            ...
            <xsl:variable name="no-side-navigation">
                <xsl:choose>
                    <xsl:when
                        test="system-data-structure/redesign/nav-options/no-side-navigation/value = 'Yes'">
            ...
            <xsl:variable name="no-quicklinks">
                <xsl:choose>
                    <xsl:when
                        test="system-data-structure/redesign/nav-options/no-quicklinks/value = 'Yes'">
            ...
            <div class="stamp {$stamp-class} type-{$content-type} {$no-top-ribbon} {$no-top-navigation} {$no-side-navigation} {$no-quicklinks}"
                data-googleanalyticsid="{$googleanalyticsid}" data-sitename="{$sitename}">
                <xsl:text>&#160;</xsl:text>
            </div>
        </xsl:for-each>
    </xsl:template>
